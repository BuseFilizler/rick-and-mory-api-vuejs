<template>
    <div class="bg-mavi pb-10">
        ...
        <div class="flex justify-center items-center gap-4">
            ...
            <p>-{{currentPage}}-</p>
            <button @click="currentPage++" v-if="(currentPage < 15)" class=" bg-yesil hover:bg-blue-400 text-white font-bold py-2 px-4 border-b-4 border-blue-700 hover:border-blue-500 rounded">Next</button>
        </div>
    </div>
</template>
<script>
...
export default {
    ...
    async mounted() {
        const data = await ramService.getApiCharacter(`/?page=1&status=dead`)
        console.log(data);
    ...
    watch: {
        async currentPage(newcurrentPage, oldcurrentPage) {
            const data = await ramService.getApiCharacter(`/?page=${newcurrentPage}&status=dead`);
            this.characters = data.results;
            this.totalPages = data.info.pages;
        }
    }
